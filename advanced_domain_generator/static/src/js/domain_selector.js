odoo.define("advanced_domain_generator.AdvancedDomainSelector", function (require) {
"use strict";
...
const relational_field = require("web.relational_fields");
...
var DomainTree = DomainNode.extend({
    ...
    _parseDomain: function (domain) {
        ...
        try {
            ...
            // currently not handled by the AdvancedDomainSelector
...
/**
 * The AdvancedDomainSelector widget can be used to build prefix char domain. It is the
 ...
 */
var AdvancedDomainSelector = DomainTree.extend({
    ...
    _initialize: function (domain) {
        ...
        // explicit. As the AdvancedDomainSelector is a specialization of a DomainTree,
    ...
    _onDomainChange: function (e) {
        ...
        // AdvancedDomainSelector catches the message and performs a full re-rendering.
...
var DomainLeaf = DomainNode.extend({
    ...
    willStart: function () {
        ...
        defs.push(this.fieldSelector.appendTo($("<div/>")).then((function () {
            ...
            if (!this.readonly) {
                ...
                // Specifific code implementation for many2one
                //OPerator in is not implemented
                // Step 1: Get proprer model and field name 
                var model = false;
                var fieldName = false;
                if (selectedField.type === 'integer' && selectedField.name === 'id') {
                    model = selectedField.model;
                    fieldName = selectedField.name;
                } else if (_.contains(["many2one", "many2many", "one2many"], selectedField.type)) {
                    model = selectedField.relation;
                    fieldName = 'id';
                }

                var fieldsInfo = {};
                fieldsInfo.form = {};
                fieldsInfo.form[fieldName] = {};
                ...
                //Find in the ancenstor a class that contain a model that is an instance of BasicModel 
                while (parent && !record) {
                    parent = parent.getParent()
                    if (parent.model && parent.model instanceof BasicModel) {
                        //makeDataPoint return the metadata to manipulate the model to instanciate the many2one widget
                        record = parent.model._makeDataPoint({
                ...
                //Finally We got the model and the record and the operator is = and != 
                if (model && record && _.contains(["=", "!="], this.operator)) {
                    //Create the relational field Many2one 
                    this.valueWidget = new relational_field.FieldMany2One(
                        this,
                        fieldName,
                        record,
                        {
                            ...
                            viewType: 'form',
                            attrs: {
                                string: selectedField.string,
                                widget: 'many2one'
                            },
                        }
                    );
                    this.valueWidget.field.relation = model;
                    //Add the new relation_field in the Leaf
                    //Use of many2one widget make the search works
                    wDefs.push(this.valueWidget.appendTo("<div/>").then((function () {
                        ...
                        if (this.value !== 0) {
                            this._rpc({
                                ...
                                if (!!result[0][0]) {
                                    self.valueWidget.value = result[0][0];  //Id of the result
                                    self.valueWidget.m2o_value = result[0][1]; //Display name 
                                    self.valueWidget._getSearchBlacklist = function () {
                                        return [result[0][0]];
                                    }
                                } else {
                                    self.valueWidget.value = 0; // If no result
                                    self.valueWidget.m2o_value = '';
                                }
                                self.valueWidget._render(); //Make the render of the widget
                            });
                        }

                        this.valueWidget.$input.addClass("o_domain_leaf_value_input");

                        //Tell the lead the value has changed
                        this.valueWidget.on('field_changed', this, function (ev) {
                            this._changeValue(ev.data.changes[this.valueWidget.name], false);
                        });
                ...
                if (selectedField.type === 'integer' && selectedField.name === 'id') {
                    model = selectedField.model;
                } else if (_.contains(["many2one", "many2many", "one2many"], selectedField.type) && selectedField.relation) {
                    model = selectedField.relation;
                }

                if (model && this.value) {
                    const values = !$.isArray(this.value) ? [this.value] : this.value;
                    wDefs.push(rpc.query({
                        model: model,
                        method: 'search_read',
                        domain: [['id', 'in', values]],
                        fields: ['display_name']
                    }).then((result => {
                        self.displayValue = result.map(line => line.display_name).join(', ');
                    }).bind(this)));
                }
            }
    ...
    _changeFieldChain: function (chain, silent) {
        ...
        this.fieldSelector.setChain(chain).then((function () {
            ...
            var selectedField = this.fieldSelector.getSelectedField() || {};
            //STD behavior : keep the previous if compatible, but id is not working if you call name get on a different
            //Model with the same id
            if (selectedField.type === 'many2one' || selectedField.name === 'id') {
                this.value = 0;
            }
    ...
    _changeValue: function (value, silent) {
        ...
        if (!silent) this.trigger_up("domain_changed", {child: self});
    },
    /**
